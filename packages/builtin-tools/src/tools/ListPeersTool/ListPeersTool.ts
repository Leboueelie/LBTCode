...
export const ListPeersTool = buildTool({
  ...
  async description() {
    return 'Discover other LBTCode sessions for cross-session messaging'
  },
  async prompt() {
    return `List active LBTCode sessions that can receive messages via SendMessage.
...
Use this tool to discover messaging targets before sending cross-session messages. Only running sessions with active messaging sockets are returned.`
  ...
  async call(_input: ListPeersInput, context) {
    ...
    const udsMessaging = require('src/utils/udsMessaging.js') as typeof import(
      'src/utils/udsMessaging.js'
    )
    const udsClient = require('src/utils/udsClient.js') as typeof import(
      'src/utils/udsClient.js'
    )
    const bridgePeers = require('src/bridge/peerSessions.js') as typeof import(
