...
import { hideIndicator, showIndicator } from '../win32/inputIndicator.js'
import {
  MODIFIER_KEYS,
  VK_MAP,
  ps,
...
function getBridge() {
  if (!_bridge) {
    try {
      _bridge = require('../win32/bridgeClient.js') as typeof import(
...
function getWm() {
  ...
  return (_wm ??= require('../win32/windowMessage.js') as typeof import(
...
const input: InputPlatform = {
  ...
  async typeText(text) {
    ...
    if (boundAppType === 'word' && boundFilePath) {
      // eslint-disable-next-line @typescript-eslint/no-require-imports
      const { appendText } = require('../win32/comWord.js') as typeof import(
        '../win32/comWord.js'
      )
...
import type { WindowAction, WindowManagementPlatform } from './types.js'
