...
export function findGitBashPath(): string {
  ...
  if (envOverride) {
    console.error(
      `LBTCode was unable to find CLAUDE_CODE_GIT_BASH_PATH path "${envOverride}"`
    )
  } else {
    console.error(
      'LBTCode on Windows requires git-bash (https://git-scm.com/downloads/win). If installed but not in PATH, set environment variable pointing to your bash.exe, similar to: CLAUDE_CODE_GIT_BASH_PATH=C:\\Program Files\\Git\\bin\\bash.exe'
    )
  }
