      ...
      <Box flexDirection="column">
        {availableIDEs.length === 0 && (
          <Text dimColor>
            {isSupportedJetBrainsTerminal()
              ...
              : 'No available IDEs detected. Make sure your IDE has the LBTCode extension or plugin installed and is running.'}
          </Text>
        )}
        ...
        {availableIDEs.length !== 0 &&
          availableIDEs.some(ide => ide.name === 'VS Code' || ide.name === 'Visual Studio Code') && (
            <Box marginTop={1}>
              <Text color="warning">Note: Only one LBTCode instance can be connected to VS Code at a time.</Text>
            </Box>
          )}
...
export async function call(
  ...
  if (args?.trim() === 'open') {
    ...
    if (availableIDEs.length === 0) {
      onDone('No IDEs with LBTCode extension detected.');
      return null;
    }
