...
import { Box, Text, wrappedRender as render } from '@anthropic/ink';
...
function Install({ onDone, force, target }: InstallProps): React.ReactNode {
  ...
  useEffect(() => {
    if (state.type === 'success') {
      ...
      setTimeout(onDone, 2000, 'LBTCode installation completed successfully', {
        display: 'system' as const,
      });
    } else if (state.type === 'error') {
      // Give error message time to render before exiting
      setTimeout(onDone, 3000, 'LBTCode installation failed', {
        display: 'system' as const,
      });
    }
  }, [state, onDone]);
  ...
  return (
    <Box flexDirection="column" marginTop={1}>
      ...
      {state.type === 'installing' && <Text color="claude">Installing LBTCode native build {state.version}...</Text>}
        ...
        <Box flexDirection="column" gap={1}>
          <Box>
            ...
            <Text color="success" bold>
              LBTCode successfully installed!
            </Text>
          </Box>
...
export const install = {
  ...
  name: 'install',
  description: 'Install LBTCode native build',
