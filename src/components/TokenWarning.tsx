...
function CollapseLabel({ upgradeMessage }: { upgradeMessage: string | null }): React.ReactNode {
  ...
  const { getStats, subscribe } = require('../services/contextCollapse/index.js') as typeof import(
...
export function TokenWarning({ tokenUsage, model }: Props): React.ReactNode {
  ...
  if (feature('CONTEXT_COLLAPSE')) {
    /* eslint-disable @typescript-eslint/no-require-imports */
    const { isContextCollapseEnabled } = require('../services/contextCollapse/index.js') as typeof import(
      '../services/contextCollapse/index.js'
    );
    /* eslint-enable @typescript-eslint/no-require-imports */
