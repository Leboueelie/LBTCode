...
import { type PromptIdentifier, getDialogConfig, getPromptIdentifier } from 'src/utils/ultraplan/prompt.js';
...
export function UltraplanLaunchDialog({ onChoice }: UltraplanLaunchDialogProps): React.ReactNode {
  ...
  const runDescription = isBridgeEnabled
    ? 'Disable remote control and launch in LBTCode on the web'
    : 'launch in LBTCode on the web';
    ...
    <Dialog title="Run ultraplan in the cloud?" subtitle={dialogConfig.timeEstimate} onCancel={handleCancel}>
      <Box flexDirection="column" gap={1}>
        <Box flexDirection="column">
          ...
          {showTermsLink ? (
            <Text dimColor>
              For more information on LBTCode on the web:
