...
import { INTERRUPT_MESSAGE, INTERRUPT_MESSAGE_FOR_TOOL_USE, extractTag } from '../../utils/messages.js';
...
export function UserTextMessage({
  ...
  if (feature('KAIROS_GITHUB_WEBHOOKS')) {
    if (param.text.startsWith('<github-webhook-activity>')) {
      ...
      const { UserGitHubWebhookMessage } = require('./UserGitHubWebhookMessage.js') as typeof import(
  ...
  if (param.text.includes(`<${FORK_BOILERPLATE_TAG}>`)) {
    /* eslint-disable @typescript-eslint/no-require-imports */
    const { UserForkBoilerplateMessage } = require('./UserForkBoilerplateMessage.js') as typeof import(
  ...
  if (feature('UDS_INBOX')) {
    if (param.text.includes('<cross-session-message')) {
      /* eslint-disable @typescript-eslint/no-require-imports */
      const { UserCrossSessionMessage } = require('./UserCrossSessionMessage.js') as typeof import(
