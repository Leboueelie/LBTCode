...
import type { FileEdit } from '@lbtcode/builtin-tools/tools/FileEditTool/types.js'
import {
  getEditsForPatch,
  getPatchForEdits,
...
export function computeEditsFromContents(
  ...
  if (singleHunk && patch.length > 1) {
    logError(
      new Error(`Unexpected number of hunks: ${patch.length}. Expected 1 hunk.`)
    )
  }
