...
Workflow 脚本的 `return` 值和终态 `RunProgress`（status / agents / phases / returnValue / error）只活在 `ProgressStore`（`src/workflow/progress/store.ts`）的内存 Map 里。一旦 LBTCode 进程关闭/重启，全部丢失。
...
新增一个 host 侧持久化模块 + 三处接入点。**引擎层 `@lbtcode/workflow-engine` 零改动**——持久化是 host 侧关注，不污染引擎接口。
...
5. **引擎层零改动** — 所有持久化逻辑在 host 侧（`src/workflow/`），引擎 `@lbtcode/workflow-engine` 接口不变。
