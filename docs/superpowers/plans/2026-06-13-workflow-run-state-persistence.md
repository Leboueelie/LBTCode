...
**Tech Stack:** TypeScript strict、Bun runtime、`node:fs/promises`（mkdir/writeFile/readdir/rename）、`bun:test`、现有 `@lbtcode/workflow-engine` 进度事件总线。
...
import 添加（在现有 `@lbtcode/workflow-engine` import 之前或之后）：
