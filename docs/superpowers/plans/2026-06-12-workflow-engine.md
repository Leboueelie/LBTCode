...
**Goal:** 把被掏空的「清单推进」版 WorkflowTool 重建为完整忠实的确定性 JS 脚本编排引擎，独立成包 `@lbtcode/workflow-engine`，通过端口适配与核心层解耦。
...
Run: `bun run --filter @lbtcode/workflow-engine test 2>&1 | head -5` 或 `cd packages/workflow-engine && bun test 2>&1 | head -5`
...
git commit -m "feat(workflow): scaffold @lbtcode/workflow-engine package"
```
...
> **里程碑：Phase 1–2 完成。** 包 `@lbtcode/workflow-engine` 现已独立可运行——全 mock 端口，无 LLM、无核心层依赖。可在此检查点整体 review。
...
```ts
import type { ProgressEvent } from '@lbtcode/workflow-engine'
...
async function runWorkflowSubAgent(
  params: AgentRunParams,
  host: import('@lbtcode/workflow-engine').HostHandle,
): Promise<AgentRunResult> {
...
const WorkflowTool = feature('WORKFLOW_SCRIPTS')
  ? (() => {
      require('@lbtcode/builtin-tools/tools/WorkflowTool/bundled/index.js').initBundledWorkflows()
      return require('@lbtcode/builtin-tools/tools/WorkflowTool/WorkflowTool.js')
        .WorkflowTool
    })()
  : null
```
...
```ts
import { WORKFLOW_TOOL_NAME } from '@lbtcode/builtin-tools/tools/WorkflowTool/constants.js'
```
