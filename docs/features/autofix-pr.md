...
用户在 fork 仓库（`feat/autonomy-lifecycle-upstream` 分支）尝试对上游 `lbtcode/claude-code#386` 跑 `/autofix-pr 386`，多次报 `git_repository source setup error`。根因：官方派发的远程 session 落在被 MCP 拒绝访问的仓库（`amdosion/claude-code-bast`），权限/可见性问题。
