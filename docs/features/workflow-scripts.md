...
> 引擎包：[`@lbtcode/workflow-engine`](../../packages/workflow-engine/)（确定性 JS 脚本编排，零核心层运行时依赖）
> 集成层：[`src/workflow/`](../../src/workflow/)

## 一、功能概述

WORKFLOW_SCRIPTS 让 LBTCode 用**确定性 JavaScript 脚本**编排多个子 agent：可分解/并行、多视角置信、规模超单上下文、可 resume/可审计。
